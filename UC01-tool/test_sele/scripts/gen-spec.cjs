...
describe("Kiểm thử tạo Quiz đa luồng - Tự động Gen từ CSV", () => {
  ...
  beforeEach(async () => {
    ...
    await browser.pause(1000); // Đợi 1 chút để UI ổn định
  });
...
validRows.forEach((row, index) => {
  ...
  const tcName = `TC_${String(index + 1).padStart(3, '0')}`;
...
`;
  ...
  if (id === 'CRQ-CRI-06') {
    ...
    // Kiểm tra UI chuyển sang được bước thêm câu hỏi
...
`;
  } else if (id === 'CRQ-NAV-06') {
      implementation = `    // Tại trang tạo quiz, chọn vào "Quiz tiêu chuẩn"
    await createQuizPage.selectQuizType("standard");
    await createQuizPage.clickContinue();

    // Vào được trang nhập thông tin cho quiz
    const titleInput = await $('[data-testid="create-quiz-title-input"]');
    const isDisplayed = await titleInput.isDisplayed();
    assert.equal(isDisplayed, true, "Vào được trang nhập thông tin cho quiz");

    console.log('✅ Đã kiểm tra luồng khởi tạo loại Quiz cơ bản (${tcName}: ${id}).');
`;
  } else if (id === 'CRQ-NAV-07') {
      implementation = `    // Tại trang tạo quiz, chọn vào "Quiz kèm tài liệu"
    await createQuizPage.selectQuizType("with-materials");
    await createQuizPage.clickContinue();
    ...
    console.log('✅ Đã kiểm tra luồng khởi tạo loại Quiz mở rộng (${tcName}: ${id}).');
`;
  }
